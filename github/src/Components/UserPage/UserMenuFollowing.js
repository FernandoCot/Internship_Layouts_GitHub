...
import FollowingBody from "./FollowingBody";


class UserMenuFollowing extends React.Component{
                ...
                {menu.map(json => (
                    <div key={json.id}>
                        <nav className="navbar_user">
                            ...
                            <Link to={"/users/"+user+"/following"} className="user_nav_item selected">Following
                                <span className="counter">{json.following}</span>
                            </Link>
                        </nav>
                    </div>
                ))}
                <FollowingBody user={user}/>    
            </div>
        )
    }
}

export default UserMenuFollowing;